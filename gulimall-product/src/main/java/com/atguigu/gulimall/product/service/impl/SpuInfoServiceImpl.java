    ...
    public void saveSpuInfo(SpuSaveVo vo) {
        ...
        if (skus != null && skus.size() > 0) {
            skus.forEach(item->{
                ...
                // 5。4 sku的优惠，满减信息 sms-》sms_sku_ladder\sms_sku_full_reduction\sms_member_price
                SkuReductionTo skuReductionTo  = new SkuReductionTo();
                BeanUtils.copyProperties(item, skuReductionTo);
                skuReductionTo.setSkuId(skuId);

                // 对满多少件优惠（满减件数）和满多少钱优惠（满减金额）进行过滤，如果件数<=0和多少钱<=0
                // ，就不走远程调用提交满减信息
                // BigDecimal不能直接比较，需要用compareTo方法，A.compareTo(B) 返回值为1时，说明A > B,
