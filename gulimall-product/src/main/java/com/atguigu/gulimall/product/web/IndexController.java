...
public class IndexController {
    ...
    //读 + 写：有读锁。写也需要等待
    //只要有些写的存在，都必须等到
    // 另外有几个线程读，就给这几个线程加几个读锁，所以在redis里能看到rw-lock下有4个读锁的key，因为有4个线程正在读
    @GetMapping("/read")
    @ResponseBody
    ...
    public String hello() {
        ...
        } finally { // 如果业务执行出错，try finally也能保证解锁成功
    ...
    private String indexPage(Model model) {
        ...
        // 返回一个model数据，里面是key-value，这样index页面就能拿到其中的数据
        model.addAttribute("categories",categoryEntities);
        return "index";
    }

    @ResponseBody   // 返回Json而不是跳转页面
    @GetMapping("index/catalog.json")
    public Map<String, List<Catelog2Vo>> getCatalogJson() {
        System.out.println("获取首页三级分类");
        Map<String, List<Catelog2Vo>> catelogJson = categoryService.getCatalogJson();
        return catelogJson;
    }
